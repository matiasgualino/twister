...
public class BytecodeCreator implements Visitor {
	...
	private int setupLocalsArray(MethodDescriptor md) {
		...
		for (int i = 0; i < md.getParameters().size(); i++) {
			VariableDescriptor vd = md.getParameters().get(i);
			vd.setOffset(++pos);
		}
	...
	private boolean isReferenceType(Type t) {
		return !(t instanceof IntegerType || t instanceof BooleanType || t instanceof StringType);
	}
	...
	private String typeDescriptor(Type t) {
		...
		else if (t instanceof StringArrayType)
			return "[java/lang/String";
	...
	public void visit(MainClass mainC) {
		...
		if (!mainC.classVars.getList().isEmpty())
			Bytecode.newline();

		for (int i = 0; i < mainC.classVars.size(); i++) {
			mainC.classVars.elementAt(i).accept(this);
		}

		Bytecode.newline2();

		for (int i = 0; i < mainC.classMethods.size(); i++) {
			Bytecode.newline2();
			mainC.classMethods.elementAt(i).accept(this);
		}
		...
		for (int i = 0; i < mainC.statements.size(); i++) {
			mainC.statements.elementAt(i).accept(this);
		}
	...
	public void visit(ClassDeclExtends classDecl) {
		...
		for (int i = 0; i < classDecl.fieldVarList.size(); i++) {
			classDecl.fieldVarList.elementAt(i).accept(this);
		}

		Bytecode.newline2();
		emitStandardConstructor(baseClassN);

		for (int i = 0; i < classDecl.methodList.size(); i++) {
			Bytecode.newline2();
			classDecl.methodList.elementAt(i).accept(this);
		}
	}

	public void visit(VarDecl varDecl) {
		for (int i = 0; i < varDecl.varL.size(); i++) {
			String fieldN = varDecl.varL.get(i).name;
	...
	public void visit(MethodDecl mdecl) {
		...
		Bytecode.directive(".method public static " + methodN + methodSig);
		...
		for (int i = 0; i < mdecl.statementList.size(); i++) {
			mdecl.statementList.elementAt(i).accept(this);
		}
		
		if (mdecl.returnExpr != null) {
	...
	public void visit(Block block) {
		for (int i = 0; i < block.stmtList.size(); i++) {
			block.stmtList.elementAt(i).accept(this);
		}
	}
	...
	public void visit(If ifStmt) {
		...
		for (int i = 0; i <  ifStmt.trueStmt.size(); i++) {
			ifStmt.trueStmt.elementAt(i).accept(this);
		}
		...
		if (ifStmt.falseStmt != null) {
			for (int i = 0; i <  ifStmt.falseStmt.size(); i++) {
				ifStmt.falseStmt.elementAt(i).accept(this);
			}
		}
	...
	public void visit(While whileStmt) {
		...
		Bytecode.label(trueL);
		for (int i = 0; i < whileStmt.stmt.size(); i++) {
			whileStmt.stmt.elementAt(i).accept(this);
		}
	...
	public void visit(Call call) {
		...
		for (int i = 0; i < call.paramExprList.size(); i++) {
			call.paramExprList.elementAt(i).accept(this);
		}
	...
	public void visit(StringLiteral strLit) {
		Bytecode.code("ldc" + strLit.value);
		incrStack();
	}
	...
	public void visit(For forStmt) {
		for (int i = 0; i < forStmt.init.size(); i++) {
			forStmt.init.elementAt(i).accept(this);
		}
		...
		for (int i = 0; i < forStmt.step.size(); i++) {
			forStmt.step.elementAt(i).accept(this);
		}
	...
	public void visit(PrintString printStr) {
		Bytecode.code("getstatic java/lang/System/out Ljava/io/PrintStream;");
		Bytecode.code("ldc \"" + printStr.str.replace("\"", "") + "\"");
		incrStack(2);
	...
	/*
	 * public void visit(GetInt getInt) {
	 * Bytecode.code("getstatic java/lang/System/in Ljava/io/PrintStream;");
	 * Bytecode.code("astore_1;");
	 * 
	 * 
	 * 
	 * Bytecode.code("ldc \"" + printStr.str.replace("\"", "") + "\"");
	...
	public void visit(StringArrayType n) {
		throw new IllegalArgumentException("BytecodeEmitter::StringArrayType");
	}

	public void visit(StringType n) {
		throw new IllegalArgumentException("BytecodeEmitter::StringType");
	}
