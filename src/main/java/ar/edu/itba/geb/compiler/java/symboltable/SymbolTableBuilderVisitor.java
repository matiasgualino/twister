...
public class SymbolTableBuilderVisitor implements Visitor {
	...
	private void error(String prefix, Identifier symbol, String posfix) {
		System.out
				.println(prefix + " " + symbol.getDescriptor() + " " + posfix);
		System.exit(1);
	}
	...
	public void visit(VarDecl varDecl) {
		...
		for (int i = varDecl.varL.size() - 1; i >= 0; i--) {
			String varName = varDecl.varL.get(i).toString();

			if (currentMethod == null) {
				if (!currentClass.addVar(varName, varT))
					error("La variable", varDecl.varL.get(i),
							" ya esta definida en esta clase");
			} else {
				if (!currentMethod.addLocalVar(varName, varT))
					error("La variable", varDecl.varL.get(i),
							" ya esta definida en este metodo");
			}
		}
	...
	public void visit(StringLiteral strLiteral) {
	}
