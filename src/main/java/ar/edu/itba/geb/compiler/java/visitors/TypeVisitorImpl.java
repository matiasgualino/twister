...
public class TypeVisitorImpl implements TypeVisitor {
	...
	private void error(Type expected, Type inferred) {
		System.out.println("Se esperaba al tipo " + expected + ", se obtuvo "
				+ inferred);
		Thread.dumpStack();
		System.exit(1);
	}
	...
	private void checkType(Type type) {
		...
		if (symbolTable.getClass(it.className) == null) {
			System.out.println("Clase desconocida: " + it.className);
			System.exit(1);
		}
	}
	...
	public Type visit(MainClass mainC) {
		...
		for (int i = mainC.classVars.size() - 1; i >= 0; i--) {
			mainC.classVars.elementAt(i).accept(this);
		}
		
		for (int i = mainC.classMethods.size() - 1; i >= 0; i--) {
	...
	public Type visit(MethodDecl methodDecl) {
		...
		if (methodDecl.returnExpr != null) {
			returnType = methodDecl.returnExpr.accept(this);
		}
	...
	public Type visit(If ifStmt) {
		...
		for (int i = ifStmt.trueStmt.size() - 1; i >= 0; i--) {
			ifStmt.trueStmt.elementAt(i).accept(this);
		}
		if (ifStmt.falseStmt != null) {
			for (int i = ifStmt.falseStmt.size() - 1; i >= 0; i--) {
	...
	public Type visit(While whileStmt) {
		...
		for (int i = whileStmt.stmt.size() - 1; i >= 0; i--) {
	...
	public Type visit(Call callStmt) {
		...
		if (method.getParameters().size() != callStmt.paramExprList.size()) {
			System.out
					.println("La cantidad de parametros del metodo no coincide con la cantidad de parametros de la llamada.");
			System.exit(1);
		}
	...
	public Type visit(StringLiteral n) {
		Type StrT = StringType.instance();
		n.setType(StrT);
		return StrT;
	}

	public Type visit(VariableOrMethod n) {
		for (VarDecl vl : n.varList.getList()) {
			vl.accept(this);
		}
